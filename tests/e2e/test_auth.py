...
def test_invalid_api_key(client):
    ...
    response = client.get("/api/v1/user/all", headers=headers)
    assert response.status_code == 403
    assert response.json() == {"detail": "Unauthorized"}


def test_valid_admin_api_key(client, admin_headers):
    response = client.get("/api/v1/user/all", headers=admin_headers)
...
def test_team_only_access(client, new_team_and_user, admin_headers):
    ...
    response = client.get(f"/api/v1/team/{team1['id']}/user/all", headers=headers)
    assert response.status_code == 200
    denied_response = client.get(f"/api/v1/team/{team2['id']}/user/all", headers=headers)
